...
@section('title') Update Service Link @endsection
@section('content')
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="live-preview">
                    <form class="row g-3 needs-validation" method="POST"  action="{{url('admin/update_serviceLink/'.$data['id'])}}" novalidate>
                        @csrf
                        <div class="mb-3">
                            <label for="validationCustom01" class="form-label">Service</label>
                             <select class="form-select" required aria-label="select example" name="service">
                                ...
                                @foreach($services as $service)
                                    <option value="{{$service['id']}}" {{$service['id'] == $data['service_id'] ? 'selected' : ""}}>{{$service['service']}}</option>
                                @endforeach
                            </select>
